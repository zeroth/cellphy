from cellphy.lib.utils import load_lib
from cellphy.lib.exceptions import LibCUDAException
...
if not cudaLib:
    ...
    try:
        # preprocessing is used when no decon is desired
...
def deskewGPU(im, dz=0.5, dr=0.102, angle=31.5, width=0, shift=0):
    """preprocessing data acquired in stage-scanning mode on GPU"""
    requireCUDAlib()
    nz, ny, nx = im.shape
